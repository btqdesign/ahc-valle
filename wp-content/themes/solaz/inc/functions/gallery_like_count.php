...
add_action('init', 'solaz_like_scripts');

add_action('wp_ajax_nopriv_solaz-post-like', 'solaz_post_like');
add_action('wp_ajax_solaz-post-like', 'solaz_post_like');

function solaz_post_like() {
    $nonce = $_POST['nonce'];
    if (!wp_verify_nonce($nonce, 'ajax-nonce'))
        die('Nope!');

    if (isset($_POST['solaz_post_like'])) {
        $like_type = 'post';
...
function solaz_getPostLikeLink($post_id, $like_type = 'post') {
    if ($like_type == 'comment') {
    ...
    $output = '<a href="#" class="solaz-post-like' . $class . '"  data-like_type="'.$like_type.'" data-post_id="' . $post_id . '" title="' . $title . '">' . $heart . $count . '</a>';
    return $output;
}
