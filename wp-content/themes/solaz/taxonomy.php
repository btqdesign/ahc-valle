...
$gallery_style = get_metadata('gallery_cat', $cat->term_id, 'gallery-style-version', true);
$gallery_col = get_metadata('gallery_cat', $cat->term_id, 'gallery-cols', true);
$btn_style = get_metadata('gallery_cat', $cat->term_id, 'gallery-loadmore-style', true);
$solaz_style_class="";
if($gallery_style == '2'){
...
$col_class="col-3";
if(isset($gallery_col) && $gallery_col != 'default'){
	$col_class = ' col-'.$gallery_col;
}elseif(isset($solaz_settings['gallery-cols'])){
    $col_class=" col-".$solaz_settings['gallery-cols'];
}
$btn = "";
if(isset($btn_style) && $btn_style != 'default'){
	if($btn_style ==2){
		$btn =' btn-default';
	}else{
		$btn ='';
	}
}elseif(isset($solaz_settings['gallery-loadmore-style']) && $solaz_settings['gallery-loadmore-style'] =='2'){
	$btn = ' btn-default';
}
?>
			...
			<div id="primary" class="content-area">
	            <?php if (have_posts()): ?>                 	
					<div class="gallery-container">
						...
						<?php endif;?> 
						<div class="tabs_sort">
							<div class="gallery-entries-wrap isotope clearfix <?php echo esc_attr($col_class).esc_attr($solaz_style_class);?>">   
				             	<?php while (have_posts()) : the_post(); ?>
				             		<?php if($gallery_style == '2'):?>
