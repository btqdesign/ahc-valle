...
if ( ! class_exists( 'solaz_reset_filter_YITH_WCAN_Reset_Navigation_Widget' ) ) {
    /**
     * YITH WooCommerce Ajax Navigation Widget
     *
     * @since 1.0.0
     */
    class solaz_reset_filter_YITH_WCAN_Reset_Navigation_Widget extends WP_Widget {

        function __construct() {
            $widget_ops  = array( 'classname' => 'yith-woocommerce-ajax-product-filter yith-woo-ajax-reset-navigation yith-woo-ajax-navigation woocommerce widget_layered_nav', 'description' => esc_html__( 'Reset all filters set by YITH WooCommerce Ajax Product Filter', 'solaz' ) );
            $control_ops = array( 'width' => 400, 'height' => 350 );
            parent::__construct( 'yith-woo-ajax-reset-navigation', esc_html__( 'YITH WooCommerce Ajax Reset Filter', 'solaz' ), $widget_ops, $control_ops );
        }
        ...
        function widget( $args, $instance ) {
            ...
            if ( count( $_chosen_attributes ) > 0 || $min_price > 0 || $max_price > 0 || apply_filters( 'yith_woocommerce_reset_filters_attributes', false ) ) {
                ...
                echo '<div class="name_label widget-content">'.esc_html__('Price : ', 'solaz').'<span>'.'&pound;'.$min_price.' - '.'&pound;'.$max_price.'</span></div>'; 
                echo "<div class='yith-wcan clear clear-all'><a class='{$button_class}' href='{$link}'>" .$label." ". "<i class='fa fa-close'></i></a></div>";
                echo $after_widget;
                echo ob_get_clean();
            }
            else {
        ...
        function form( $instance ) {
            ...
            $defaults = array(
                'title' => '',
                'label' => esc_html__( 'Reset All Filters', 'solaz' )
            );
            ...
            <p>
                <label>
                    <strong><?php esc_html_e( 'Title', 'solaz' ) ?>:</strong><br />
                    <input class="widefat" type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" />
                </label>
            </p>
            <p>
                <label>
                    <strong><?php esc_html_e( 'Button Label', 'solaz' ) ?>:</strong><br />
