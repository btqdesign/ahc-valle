 ...
 */
$thumbnail = '';
$title = $source = $type = $col_num= $onclick = $custom_links = $custom_links_target = $img_size = $external_img_size = $images = $custom_srcs = $el_class = $interval = $css = $css_animation = '';
$large_img_src = '';
...
if ( 'nivo' === $type ) {
	...
	$col_num_class='';
	if($col_num!='default'){
		$col_num_class =' gallery_col'.esc_attr($col_num);
	}
	$el_start = '<li class="isotope-item">';
	$el_end = '</li>';
	$slides_wrap_start = '<ul class="wpb_image_grid_ul'.esc_attr($col_num_class).'">';
	$slides_wrap_end = '</ul>';
}
...
foreach ( $images as $i => $image ) {
	...
	$gal_images .= $el_start; 
	$gal_images .='<div class="gallery_relative">';

	$gal_images .=  $thumbnail  .$link_start.'+'.$link_end;
	$gal_images .='</div>';
	$gal_images .= $el_end;
}
